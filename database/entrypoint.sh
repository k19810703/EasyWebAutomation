...
if [[ $1 = "db2start" ]]; then
  echo "CONNECT TO AUTOTEST USER db2inst1 using ${DB2INST1_PASSWORD};" > /usr/src/DDL.sql
  cat /usr/src/AutoTest_DDL.sql >> /usr/src/DDL.sql
  echo "CONNECT TO AUTOTEST USER db2inst1 using ${DB2INST1_PASSWORD};" > /usr/src/CONFIG.sql
  cat /usr/src/T_CONFIG.sql >> /usr/src/CONFIG.sql
